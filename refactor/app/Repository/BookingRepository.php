...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
     ...
     * @return mixed
     */
    public function storeJobEmail(JobStoreRequest $request)
    {
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        $date = Helper::formatDate($job->due);
        $time = Helper::formatTime($job->due);
        $duration = Helper::convertHoursToMinutes($job->duration);
        $jobId = $job->id;
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function updateJob($id, $data, $cuser)
    {
        ...
        $changeStatus = \App\Models\Job::changeStatus($job, $data, $changeTranslator['translatorChanged']);
        ...
        */
        ...
        else
        {
            if ($job->due->diffInHours(Carbon::now()) > 24)
            {
                ...
                $data = \App\Models\Job::jobToData($job);

                // send Push all suitable translators
                $this->sendNotificationTranslator($job, $data, $translator->id);
                $response['status'] = Constant::HTTP_RESPONSE_STATUSES['success']['label'];
            }
            else
            {
                $response['status'] = Constant::HTTP_RESPONSE_STATUSES['failed']['label'];
                $response['message'] = __('messages.jobs.failed');
            }
        }
        return $response;
    }
